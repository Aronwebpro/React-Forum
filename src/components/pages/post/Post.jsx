...
class Post extends Component {
                        ...
                        <div className="full-post new-post-body">
                            {	//Return comment text if quote
                                ...
                                (<div><span className="theme-color_txt">Replying to...</span><br/>
                                    <div className="quote"><p>{this.state.replyText.user} said: </p>
                                        <p>"{this.state.replyText.text}"</p></div>
                                </div>)
                            }
                            <form>
                                <label htmlFor=""><h2>Write a Comment:</h2></label>
                                <textarea name="" id="" cols="30" rows="10"
                                          ref={(input) => this.respText = input}></textarea>
                            </form>
    ...
    respondText = (data) => {
        ...
        if (this.state.reply === true) {
            ...
            if (this.state.clickedId !== data.commentId) {
            ...
            this.setState({
                replyText: {text: data.text, user: data.user},
                reply: false,
                replyStyleInit: {display: 'none'}
            });
            this.respond();
                                ...
                                <div className="full-post">
                                    <div className="post">
                                        <div className="post-info"><span
                                            className="theme-color_txt">Category:</span> {post.category}, <span
                                            className="theme-color_txt">Posted:</span> {createdDate + ' ' + createdTime}
                                            <span className="topic-type"> {post.type && (
                                                <span>#{post.type}</span>)}</span></div>
                                    ...
                                    <div className="author-info">
                                        ...
                                        <p><span className="theme-color_txt">Author:</span> <span
                                            className="bold">{post.authorName}</span></p>
                                        <p><span className="theme-color_txt">Member Since:</span> <span
                                            className="bold">{createdDate}</span></p>
                                    </div>
                                    <div className="fl_c"/>
                                </div>
